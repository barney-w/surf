...
async def chat(body: ChatRequest, request: Request) -> JSONResponse:
    ...
    agent_response, _ = await process_agent_response(
        agent_response, rag_outputs, routed_agent
    )
...
async def chat_stream(body: ChatRequest, request: Request) -> StreamingResponse:
    ...
    def _sse_error(code: str, msg: str, *, retryable: bool = True) -> str:
        return sse({"type": "error", "error": {"code": code, "message": msg, "retryable": retryable}})

    async def _stream_error(code: str, msg: str, *, retryable: bool = False) -> AsyncGenerator[str, None]:
        ...
        try:
            while True:
                ...
                if kind == "error":
                    ...
                    if is_prompt_too_long(exc):
                        yield _sse_error("PROMPT_TOO_LONG", PROMPT_TOO_LONG_MESSAGE, retryable=False)
